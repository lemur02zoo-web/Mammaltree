...
// ── Config ───────────────────────────────────────────────────────────────────
// Replace with your Cloudflare Worker URL after deploying the worker
// See CLOUDFLARE_WORKER_SETUP.md for instructions
const IUCN_PROXY_URL = "https://iucn-proxy.lemur02zoo.workers.dev";
...
function useIUCN(sciName) {
  ...
  useEffect(() => {
    ...
    // Calls your Cloudflare Worker proxy (free, no outbound restrictions)
    // Worker URL set in IUCN_PROXY_URL constant at top of file
    const encoded = encodeURIComponent(sciName.trim());
    fetch(`${IUCN_PROXY_URL}/taxa/scientific_name/${encoded}`)
      .then(r => r.json())
      .then(d => {
        if (d?.taxon) {
          setState({ data: d, loading: false, error: null });
        } else if (d?.error) {
          setState({ data: null, loading: false, error: d.error + (d.detail ? `: ${d.detail}` : "") });
        } else {
...
function IUCNPanel({ sciName }) {
  ...
  const sisUrl  = taxon.sis_id ? `https://www.iucnredlist.org/species/${taxon.sis_id}` : null;
  ...
  return (
    <div>
      ...
      <Row label="Criteria"          value={a.criteria} />
      <Row label="Class"             value={taxon.class_name} />
      <Row label="Order"             value={taxon.order_name} />
      <Row label="Family"            value={taxon.family_name} />
      {sisUrl && (
        <a href={sisUrl} target="_blank" rel="noreferrer"
